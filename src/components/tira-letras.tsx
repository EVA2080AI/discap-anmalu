...
export type PasoTira = { letra: string; avatar: AvatarId };
  ...
  return (
    <div className="flex flex-wrap justify-center gap-1.5" role={onElegir ? "listbox" : undefined} aria-label="Letras de la palabra">
      {pasos.map((p, i) => {
        ...
        const src = esEspacio ? null : foto(p.avatar, p.letra) ?? senaFormal(p.letra);
        ...
        return (
          <Comp
            ...
            aria-label={esEspacio ? "espacio" : `letra ${p.letra}`}
            className={cn(
          ...
          >
            ...
            {!esEspacio && (
              <span className={cn("absolute bottom-0 left-0 right-0 bg-white/85 text-center font-display font-extrabold leading-tight text-navy", tamano === "sm" ? "text-xs" : "text-sm")}>{p.letra}</span>
            )}
          </Comp>
        );
      })}
